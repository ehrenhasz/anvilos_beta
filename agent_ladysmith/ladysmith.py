...
SYS_INSTRUCT = (
    ...
    "1. MICRO-CHUNKING: Do not boil the ocean. Do not try to port entire apps in one go. "
    "   - BAD: 'Port Vim to Anvil'\n"
    "   - GOOD: 'Implement TextBuffer class' -> 'Implement Cursor' -> 'Implement Render'\n"
    "2. SIZE LIMIT: If a card requires >100 lines of Python, SPLIT IT.\n"
    "3. VERIFICATION: Every card must include a test function to verify itself.\n"
    "4. NO HALLUCINATION: Use standard MicroPython/Anvil libs only. No external pip packages.\n"
    "5. RUNTIME: All verification must use the system binary: /usr/local/bin/anvil\n"
    "6. FORMAT: Use 'PYTHON_SCRIPT' for logic. Use 'SYSTEM_OP' only for file/shell ops.\n"
    "\n"
    "Your goal is to generate the cards that 'Aimeat' (The Smith) will execute."
)
